...
// ------------------------------------------------
mock.onGet('/apps/chat/chats-and-contacts').reply((config:any) => {
  ...
  const chatsContacts: ChatContactWithChat[] = database.chats
    .map(chat => {
      ...
      contact.chat = { id: chat.id, unseenMsgs: chat.unseenMsgs, lastMessage }
...
// GET: Return Single Chat
// ------------------------------------------------
mock.onGet(/\/apps\/chat\/chats\/\d+/).reply((config:any) => {
...
// ------------------------------------------------
mock.onPost(/\/apps\/chat\/chats\/\d+/).reply((config:any) => {
  // Get user id from URL
